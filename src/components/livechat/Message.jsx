...
function Message({message,user1}) {
  ...
  return (
    <div
    className={isMe(message)?"isent":"incoming"}
    ref={scrollRef}
    >
        
       { message.active? <>{isMe(message)
         
         ?<SentBox>
            {message.media?<img
            className={`image ${isMe(message)? "fromme":"fromadmin"}`}
            src={message.media} alt=""/>:null}
            <p>{message.text}</p>
            <br/>
            <small>
                <Moment 
                fromNow  
                >{message.createdAt.toDate()}</Moment>
            </small>
            </SentBox>
          :<InComeBox>
                {message.media?

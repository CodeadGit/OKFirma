...
const MyJobCenter = ({ job, products }) => {
  ...
  const sendEmailToUser = async () => {
    ...
    await emailjs.send("onlinekesif_support", "template_fd5d0vb", params).then(
      (result) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const allNotesDefined =
      noteList.length > 0 ? checkPriceDefined(noteList, "note") : true;
    if (allNotesDefined) {
      if (allPricesDefined) {
        if (!conditions.deliveryTime) {
          ...
          if (!conditions.expiration) {
            ...
            if (!conditions.recruitment) {
              ...
              var objected = thisProducts.reduce(
                (a, v) => ({ ...a, [v.id]: v }),
                {}
              );
              let a, b, c, d, e, f;
            ...
            return (
              <>
                {Array.isArray(job.wishDetail[i]) ? (
                  ...
                  <div key={idx} className="section-individual">
                    <span className="row-title sub" key={idx}>
                      {job.wishDetail[i].q}
                    </span>
                    <span className="dots">:</span>
                    <span className="row-text" key={idx}>
                      {job.wishDetail[i].a}
                    </span>
                  </div>
                )}

import { AttachEmail, AttachFile, CloseOutlined } from '@mui/icons-material'
...
function NewSupport({setAlert,setError}) {
    ...
    if(file){
        ...
        const imgRef=ref(storage,`requests/${new Date().getTime()+file.name}`)
        const snap=await uploadBytes(imgRef,file)
        const dlurl=await getDownloadURL(ref(storage,snap.ref.fullPath)).catch(e=>{setError(e.message)
        setUploading(false)
        })
        url=dlurl;
        ...
        await addDoc(docRef,{
            ...
            subject:selected,
            priority:priority,
            content:[{text:request.text,createdAt:Timestamp.fromDate(new Date()),media:url||"",from:user}],
        ...
        .then(()=>setAlert("Talebinizi aldık, 'Destek Talebi' sekmesinden ilgili talebinizi takip edebilirsiniz."))    
        .then(()=>{setRequest("")
                    setFile("")}).finally(()=>navigate("/mesajlarim/Destek-Talebi"))
            .catch((e)=>{
                setUploading(false)
                setError(e.message)})
    ...
    const caseList=["Kombi","Klima","Kazan","Doğalgaz Tesisat","Doğalgaz Projesi","Fatura","Hesap","Genel"]
  ...
  return (
    <div className='newSupportArea'>
        ...
        {
        uploading?<Loading title="talebiniz gönderiliyor"/>
        :<form
        ...
        <div className="support-buttons-left">
            ...
            onClick={()=>setFile("")}
            className='icon'/>
            <small>{file.name}</small></div>:null}
        </div>

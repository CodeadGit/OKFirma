    ...
    useEffect(()=>{
        ...
        if(user&&userData.firm&&!userData.client){
            ...
            onSnapshot(q,(doc)=>{
                ...
                if(jobs.length>0){
                    var first=jobs.filter(i=>new Date().getTime()<new Date(i.publishRemaining.seconds*1000).getTime())
                    var second=first.filter(i=>!i.interestedFirms.includes(auth?.currentUser.uid))
                    var third=second.filter(i=>i.statue===0||i.statue===1||i.statue===2)
...
//get my jobs que
    useEffect(()=>{
        if(user){
            let userid=user.uid
            const q = query(collection(db, "Jobs"), where("interestedFirms","array-contains", auth.currentUser.uid))
            onSnapshot(q,(snap)=>setMyJobsData(snap.docs.map(doc=>doc.data())))
    ...
    useEffect(()=>{
    if(user){
        const q = query(collection(db, "Offers"), where("firm", "==", auth.currentUser.uid))
    ...
    useEffect(()=>{
    if(user){
        const q = query(collection(db, "ClientsOfFirms"), where("holder", "==", auth.currentUser.uid))
    ...
    useEffect(()=>{
    if(user){
        const q = query(collection(db, "ProductsOfFirms"), where("holder", "==", auth.currentUser.uid))
        onSnapshot(q,(snap)=>setMyFirms(snap.docs.map(doc=>doc.data())))
    ...
    return(
        <CloudContext.Provider value={{myJobs,myPossibleJobs,myJobsData,myFirms,myProducts,news,favoriArea}}>
            {children}
